...
function editTransaction(id) {
  ...
  if (editMode) {
    document.querySelector(
      "#dynamic-buttons"
    ).innerHTML = `<button class="btn btn-success my-3" onclick="editHandler(${id})">Edit Transaction</button>
    <button class="btn btn-danger my-3" onclick="viewHandler()">Cancel Edit</button>
    `;
  }
}
...
function viewHandler() {
  ...
  let str = transactions
    .map((ele, i) => {
      ...
      return `
         <li class="my-1 p-2">${ele.title} ${ele.amount} 
